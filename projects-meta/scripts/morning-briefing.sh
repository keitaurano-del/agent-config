...
# ============================================================
# Daily Note 本体生成（briefing 後に実行）
# ============================================================
DAILY_OUT="${VAULT_DIR}/50-Daily/${DATE}.md"
DAILY_TPL="${VAULT_DIR}/90-Templates/daily-template.md"

# 当日の Daily Note が既に存在する場合はスキップ（手動編集を保護）
if [[ -f "$DAILY_OUT" ]]; then
  echo "[$(date)] Daily Note already exists, skipping: $DAILY_OUT"
else
  BRIEFING_CONTENT=$(cat "$OUTPUT" 2>/dev/null || echo "（朝ブリなし）")
  WEEKDAY=$(date +%A)

  echo "[$(date)] Daily Note generating..."

  claude --print --agent ceo <<PROMPT_DAILY > "$DAILY_OUT" 2>&1
今日 ${DATE}（${WEEKDAY}）の Obsidian Daily Note を生成してくれ。
...
ルール:
- frontmatter: date: ${DATE}, weekday: ${WEEKDAY}, updated_by: masayoshi-ceo
- 見出し構成（今日の Top 3 / 気になっていること / 進捗ハイライト / 夜の振り返り / 関連リンク）を維持
- 夜の振り返り欄は空欄のまま（寝る前の手動記入用）
- 朝ブリの要点を簡潔に統合（全文コピーでなく要約）
- 中立的丁寧体 + 装飾記号 ** なし

応答はこの md ファイルそのもの。ファイル保存・ツール実行・説明は不要。
PROMPT_DAILY

  # Daily Note も同時に commit
  git add "50-Daily/${DATE}.md" 2>/dev/null || true
  if ! git diff --cached --quiet; then
    git commit -m "daily-note(${DATE}): auto-generated by morning-briefing" || true
    git push origin main || echo "[$(date)] Daily Note push failed, will retry next run"
  fi

  echo "[$(date)] Daily Note generated: $DAILY_OUT"
fi
